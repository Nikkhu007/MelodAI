...
// Handle 401 globally (skip for refresh endpoint)
...
export const lyricsAPI = {
  get: (artist, title) => api.get('/lyrics', { params: { artist, title } }),
}

// ── Simple in-memory request cache ─────────────────────────────────────────
const apiCache = new Map()

export function cachedGet(url, params, ttlMs = 5 * 60 * 1000) {
  const key = url + JSON.stringify(params || {})
  const cached = apiCache.get(key)
  if (cached && Date.now() < cached.expiry) return Promise.resolve(cached.data)
  return api.get(url, { params }).then(res => {
    apiCache.set(key, { data: res, expiry: Date.now() + ttlMs })
    return res
  })
}
